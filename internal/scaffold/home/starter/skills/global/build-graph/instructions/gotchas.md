...
- The `.code-review-graph/` directory may not exist on first run — use `da kg code-status` to see whether a full `kg build` is needed.

## Stale Graph After Branch Switch

- Switching branches does not always rebuild the graph automatically.
- After `git checkout` to a very different branch, prefer `da kg build` (full) or `kg update` after reviewing `kg code-status`.
...
- If you see stale paths after renames, run a full `da kg build` once.
