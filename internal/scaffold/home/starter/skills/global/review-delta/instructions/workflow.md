...
Use **`da kg`** subcommands from the repository root unless you pass `--repo`.
...
```bash
da kg bridge query --intent callers_of "pkg.Type.Method"
da kg bridge query --intent tests_for "pkg.Type.Method"
da kg bridge query --intent callees_of "pkg.Type.Method"
```
...
`da kg serve` exposes the same operations as JSON-RPC tools (`build_or_update_graph_tool`, `get_review_context_tool`, `query_graph_tool`, etc.). Prefer **CLI** in agent sessions; use MCP only when the host environment requires it.
