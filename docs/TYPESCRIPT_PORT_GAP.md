...
| `review` | absent | **Phase 4 opt-out** | Go owns the scope-routed proposal review surface (`show`/`approve`/`reject`) across project-local `.agents/proposals/` and higher-scope proposal stores such as `~/.agents/proposals/` for user scope. Approved proposals write into the routed scope root. This is part of the orchestration / shared-store mutation surface; boundary §"Permanently deferred" keeps full orchestration parity in Go. |
