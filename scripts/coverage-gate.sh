...
# - vendor/ — third-party.
# - internal/testutil, internal/linktest — test-scaffolding helpers that
#   wrap *testing.T; failure branches call t.Fatal on the caller's T and
#   cannot be exercised from a Go test (Go does not permit constructing or
#   recovering from a foreign *testing.T). Coverage of the link primitives
#   they wrap is exercised through downstream callers and links own tests.
...
EXCLUDE_RE="${COVERAGE_EXCLUDE:-^(github.com/[^/]+/[^/]+/cmd/[^/]+|.*/internal/storetest|.*/internal/testutil|.*/internal/linktest|.*/internal/scaffold/(home|hooks|templates)|.*/vendor/.*)$}"
