...
Status: ALL PHASES COMPLETE (2026-04-12); Phases 3–6 verified; full test suite green
...
## Phase 3: Centralize Shared Repo Targets First ✓ COMPLETE (2026-04-12)
...
- [x] Migrate the highest-conflict repo-local outputs onto the shared executor first (2026-04-11):
  - `.agents/skills/<name>` — all 4 platform adapters emit via `BuildSharedSkillMirrorIntents`
  - `.claude/skills/<name>` — Claude adapter emits skills mirror for `.claude/skills/` root
  - `.claude/settings.local.json` — platform-owned; Claude/Copilot adapters handle directly (not a shared target conflict in practice)
- [x] Deduplicate identical intents and fail fast on incompatible intents for the same path (2026-04-11 — `BuildResourcePlan` dedupes compatible intents and returns error for conflicting intents; tests: `TestBuildSharedTargetPlanDedupesIdenticalIntentsAcrossPlatforms`, `TestBuildSharedTargetPlanRejectsConflictingIntentsAcrossPlatforms`).
- [x] Add safe directory replacement only in the centralized executor for approved shared targets (2026-04-11 — `prepareIntentTargetForReplacement` + `removeImportedDirIfAllowlisted` + `isAllowlistedSharedMirrorTarget`; tests in Phase 6 verification).
- [x] Extend the same planner/executor framework to canonical `agents/` projections (2026-04-11):
  - `.claude/agents/<name>` — `BuildSharedAgentMirrorIntents` via Claude + Cursor adapters
  - `.codex/agents/*.toml` — `BuildSharedCodexAgentTomlIntents` via Codex adapter
  - `.opencode/agent/*.md` — `BuildSharedAgentFileSymlinkIntents` via OpenCode adapter
  - `.github/agents/*.agent.md` — `BuildSharedAgentFileSymlinkIntents` via Copilot adapter
...
- [x] Apply the same adapter-thinning to `agents/` projections: all platform adapters delegate agents/ writes to `CollectAndExecuteSharedTargetPlan`; `createAgentsLinks` stubs return nil (2026-04-11).
...
- [x] Run focused packages first, then `go test ./...` (2026-04-12 — `go test ./internal/platform/...`: ok; `go test ./...`: all packages green).
